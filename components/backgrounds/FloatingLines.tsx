...
const vec3 BLUE  = vec3(70.0, 120.0, 240.0) / 255.0;
...
vec3 background_color(vec2 uv) {
  ...
  float m = uv.y - y;
  col += mix(PINK, BLACK, smoothstep(0.0, 0.6, abs(m))) * 0.4;
  col += mix(BLUE, BLACK, smoothstep(0.0, 1.0, abs(m - 0.5))) * 1.2;
  return col * 0.5;
}
...
export default function FloatingLines({
  ...
  const midLD = enabledWaves.includes('middle') ? getLineDistance('middle') * 0.01 : 0.01
  const botLD = enabledWaves.includes('bottom') ? getLineDistance('bottom') * 0.01 : 0.01
  ...
  }, [linesGradient, enabledWaves, animationSpeed, interactive, bendRadius, bendStrength, mouseDamping, parallax, parallaxStrength])
